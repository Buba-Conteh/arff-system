<div class="w-full mt-4 p-4 box-border" x-data="{open:false, isUpdate:false}" >
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                          ...
                          <div class="grid grid-cols-3 ">
                              ...
                              <div class="col-span-6 md:col-span-4">
                                <label for="time" class="block text-sm font-medium text-gray-700">Short Hand</label>
                                <input wire:model.lazy="short_hand" type="text"
                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                >
                                @error('short_hand') <span class="error text-red-500">{{ $message }}</span> @enderror
    ...
    <p class="text-xl pb-3 flex items-center">
     <i class="fas fa-list mr-3"></i> Ranks
    </p>
    <div class="flex justify-end sm:px-6 lg:px-8 py-4 ">
        <a href="#" @click.prevent="open = !open; isUpdate=false">
        <button class="bg-green-400 p-4 w-full cta-btn font-semibold py-2 mt-2 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl hover:bg-gray-300 flex items-center justify-center" >New Rank</button>
        </a>
    </div>
        ...
        <table class="text-left w-full border-collapse"> <!--Border collapse doesn't work on this site yet but it's available in newer tailwind versions -->
            <thead>
                ...
                <tr>
                    ...
                    <th class="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">short_hand</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-grey-lighter">
                    <td class="py-4 px-6 border-b border-grey-light">{{$rank->name}}</td>
                    <td class="py-4 px-6 border-b border-grey-light">{{$rank->short_hand}}</td>
                    <td class="py-4 px-6 border-b border-grey-light">{{$rank->created_at}}</td>
                    
                    <td class="py-4 px-4 border-b border-grey-light">
                        <a href="#" class="text-indigo-600 hover:text-indigo-900">
                          <i class="fas fa-eye"></i>
                        </a>
                      <a href="#" @click="open = !open; isUpdate= true" wire:click.prevent="edit({{$rank->id}})" class="text-green-600 hover:text-green-900">
